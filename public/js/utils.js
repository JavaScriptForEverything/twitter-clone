...
const getTweetHTML = (tweet, { isModal=false } = {}) => {
	...
	return `<div class='tweet-container' id=${tweet._id}>
		...
		${tweet.retweet ?  `
			<h2 
				class='mb-1 text-slate-600 hover:text-slate-700 px-4 text-xs font-medium uppercase'
			>
		...
		<div class='px-4 py-2 text-slate-700 flex gap-4'>
			...
			<div name='pin-label-container' class='[&>button]:mt-2 flex-1'>
				...
				<div class='flex gap-0.5 items-center'>
					...
					${isModal ? '' : `
							<button id='pin-button' name='pin-button' type='button' data-pinned='${tweet.pinned ? 'pinned': ''}'
